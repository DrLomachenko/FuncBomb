...
\begin{enumerate}
    ...
    \item По лемме Рисса о почти перпендикуляре выбираем $y_n\in L_n$, $\|y_n\|=1$, так что $\rho(y_n,L_{n-1})>\frac12$.
    ...
    \item Следовательно, $\forall r>0$ множество $\{\lambda\in\sigma(A):|\lambda|>r\}$ конечно. Тогда $\sigma(A)\setminus\{0\}=\bigcup_{n=1}^{\infty}\{\lambda\in\sigma(A):|\lambda|>\frac1n\}$ не более чем счётно, и если множество $\{ \lambda_k\}$ счётно, то $\lambda_k \rightarrow 0, k \rightarrow \infty$.
