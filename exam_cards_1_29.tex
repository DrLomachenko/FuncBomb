...
\begin{document}
	...
	\begin{enumerate}
		...
		\item $\forall f \in X^* \hookrightarrow f(K) \subset \CC$ -- компакт как непрерывный образ компакта, а значит ограничено. Тогда $\Phi(x_n) \in X^{**}$ ограничены поточечно, а значит по Банаху--Штейнгаузу ограничены нормы $\|\Phi(x_n)\| = \|x_n\|$.

		\item $\forall k \hookrightarrow \{f_k(x_n)\} \subseteq B_M[0] \subset \CC$. Последовательность вложена в компакт, значит $\forall k\; \exists \{n_m^{(k)}\}: f_k(x_{n_m^{(k)}}) \rightarrow a_k$. Диагональным процессом выделяем подпоследовательность $\{x_{n_m}\}$ такую, что для каждого $k$ последовательность $f_k(x_{n_m})$ сходится.
